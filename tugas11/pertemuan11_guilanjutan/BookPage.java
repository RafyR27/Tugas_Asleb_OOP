...
//        btnDelete.setEnabled(frame.isAdmin());
        ...
        btnDelete.addActionListener(e -> {
            ...
            if(row >= 0){
                Book book = frame.getBooks().get(row);
                int result = JOptionPane.showConfirmDialog(this, "Anda yakin ingin menghapus buku " + book.getJudul() , "Hapus Buku", JOptionPane.OK_CANCEL_OPTION);
                if(result == JOptionPane.OK_OPTION){
                    book = frame.getBooks().remove(row);
                    refreshTable(frame.getBooks()); 
                }
            }
        });
